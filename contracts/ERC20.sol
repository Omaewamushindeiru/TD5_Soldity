...
contract ERC20 is IERC20 {
    ...
    mapping(uint256 => address) listAddressWhitelisted;
    ...
    constructor() public {
        ...
        decimals = 18;
        totalWhitelisted = 0;
        _totalSupply = 5858268;
        balances[msg.sender] == _totalSupply;
        whitelist[msg.sender] = true;
    }
//owner dans whitelisted
    // owner approves buyers by address when they pass the whitelisting procedure
    function addToWhiteList(address addr) public {
        require(msg.sender == contract_owner);
        listAddressWhitelisted[totalWhitelisted] = addr;
        totalWhitelisted++;
        whitelist[addr] = true;
    }

    // quantity is the quantity of token given per addresses
    function airdrop(uint256 quantity) public {
        require(msg.sender == contract_owner);
        require(quantity * totalWhitelisted <= balanceOf(contract_owner) && quantity > 0);

        for(uint256 i = 0; i < totalWhitelisted; i++){
            transfer(listAddressWhitelisted[i], quantity);
        }
    }
